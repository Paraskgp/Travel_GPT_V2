...
- **[DONE 2026-05-29]** Map phase rate limiting — revised: `MAP_CONCURRENCY = 20` with no inter-batch delay drains the Gemini free-tier RPM quota entirely during the map phase, leaving no quota for the dedup phase (both chunks immediately 429 and the pipeline returns []). Fixed: `MAP_CONCURRENCY = 5`, `MAP_BATCH_DELAY_MS = 5_000` added to `runWithConcurrency`. Dedup chunks serialized (sequential, not parallel) with a `DEDUP_CHUNK_DELAY_MS = 5_000` gap. Net result: map phase spreads calls over a longer window (~14 min instead of ~6 min), leaving quota for dedup. At paid Gemini tier the delays are still applied but 429s never occur so the run finishes in the same wall-clock time.
